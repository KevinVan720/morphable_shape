...
class TrapezoidShape extends Shape {
  ...
  const TrapezoidShape({this.side=ShapeSide.bottom, this.inset=const Length(20, unit: LengthUnit.percent)});
      ...
      case ShapeSide.bottom:
        {
          nodes.add(DynamicNode(position: Offset(0,0)));
          nodes.add(DynamicNode(position: Offset(size.width,0)));
          nodes.add(DynamicNode(position: Offset(size.width-inset, size.height)));
      ...
      case ShapeSide.right:
        {
          ...
          nodes.add(DynamicNode(position: Offset(size.width, size.height-inset)));
