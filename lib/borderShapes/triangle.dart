...
class TriangleShape extends Shape {
  ...
  Path generatePath(
    ...
    Path path = Path();
    if (point1.dx < 1) {
      path.moveTo(point1.dx * width, 0);
    } else if (point1.dx == 1) {
      path.moveTo(width, point1.dy * height);
    } else {
      path.moveTo(width * (point1.dx - 1), height);
    }
    if (point2.dx < 1) {
      path.lineTo(point2.dx * width, 0);
    } else if (point2.dx == 1) {
      path.lineTo(width, point2.dy * height);
    } else if (point2.dy == 1) {
      path.lineTo(width * (point2.dx - 1), height);
    } else {
      path.lineTo(0, height * (2 - point2.dy));
    }
    if (point3.dx == 1) {
      path.lineTo(width, point3.dy * height);
    } else if (point3.dy == 1) {
      path.lineTo(width * (point3.dx - 1), height);
    } else {
      path.lineTo(0, height * (2 - point3.dy));
    }
