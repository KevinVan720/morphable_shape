...
enum ShapeCorner { topLeft, topRight, bottomLeft, bottomRight, leftTop, leftBottom, rightTop, rightBottom}
...
Offset ArcPoint(Rect rect, double t) {
  double xc = rect.center.dx, yc = rect.center.dy, rx = rect.width / 2, ry = rect.height / 2;
...
List<Offset> arcToCubicBezier(Rect rect, double startAngle, double sweepAngle,
  ...
  double xc = rect.center.dx, yc = rect.center.dy, alpha;

  List<Offset> rst = [];
  Offset p1, p2, p3, p4;
  p1=ArcPoint(rect, startAngle);
  p4=ArcPoint(rect, startAngle+sweepAngle);
  alpha=sin(sweepAngle)*(sqrt(4+3*tan(sweepAngle/2)*tan(sweepAngle/2))-1)/3;
...
extension addDynamicNodeExtension on List<DynamicNode> {
  void cubicTo(Offset x1, Offset x2, Offset x3) {
    ...
    this.last.nextControlPoints=x1;
    DynamicNode newNode=DynamicNode(position: x3, prevControlPoints: x2);
    this.add(newNode);
  }
